...
def _get_full_name(node):
    """
    Recursively get the full name of an AST node, handling attributes.
    """
    if isinstance(node, ast.Name):
        ...
        # Handle cases like MyMetaclass(BaseClass)
        if isinstance(node.func, ast.Name):
            return node.func.id
        elif isinstance(node.func, ast.Attribute):
            return f"{_get_full_name(node.func.value)}.{node.func.attr}"
    return None
