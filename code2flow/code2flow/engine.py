...
def generate_image(file_groups, all_nodes, edges, output_filename, hide_legend=True, no_grouping=False):
    """
    Generates a PNG image from the graph data.
    """
    if not is_installed('dot') and not is_installed('dot.exe'):
        raise AssertionError(
            "Can't generate a flowchart image because neither `dot` nor "
            "`dot.exe` was found. Please install graphviz.")

    # Create a temporary dot file
    temp_dot_file = "temp_graph.dot"
    with open(temp_dot_file, 'w') as fh:
        write_file(fh, nodes=all_nodes, edges=edges,
                   groups=file_groups, hide_legend=hide_legend,
                   no_grouping=no_grouping)

    # Convert dot to png
    _generate_graphviz(temp_dot_file, "png", output_filename)
    logging.info("Completed your flowchart! To see it, open %r.", output_filename)
...
def code2flow(raw_source_paths, language=None, hide_legend=True,
